...
| LLM content generation (Groq) | **Working well for both platforms** | 4 nodes call Groq's `llama-3.3-70b-versatile` via raw HTTP Request + hardcoded API key. Instagram's prompts needed a harder rewrite (FLUX-specific + few-shot examples) than LinkedIn's — see `history.md`. |
